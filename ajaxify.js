...
class Ajaxify {
  constructor(options) {
    ...
    Hints.prototype.find = function (t) {
      return !t || !this.myHints ? false : this.myHints.some((h) => t.iO(h));
    }; //iterate through hints within passed text (t)

    function lg(m) {
      $.s.verbosity && console && console.log(m);
    }
    ...
    class GetPage {
      constructor() {
        ...
        let _lSel = ($t) => (
          ...
          _parseHTML = (h) =>
            (document.createElement("html").innerHTML = _replD(h).trim()),
          _replD = (h) =>
            String(h)
              .replace(docType, "")
              .replace(tagso, div12)
      ...
      constructor() {
        ...
        let _allstyle = ($s) =>
          ...
          _onetxt = ($s) =>
            ...
            !txt.iO("new Ajaxify(") &&
            (($.s.inline && !Hints($.s.inlineskip).find(txt)) ||
          ...
          _apptxt = ($s) => {
            ...
            try {
              sc.appendChild(document.createTextNode($s.textContent));
            } catch (e) {
          ...
          _addstyle = (t) =>
            qs("head").appendChild($.parse("<style>" + t + "</style>")),
          _addScripts = ($s) => (
            $.addAll($s.c, "href"),
            $.s.inlinesync
              ? setTimeout(() => $.addAll($s.j, "src"))
              : $.addAll($s.j, "src")
          );
      }
    }
    ...
    class DetScripts {
      constructor() {
        ...
        let _rel = (lk, v) =>
          Array.prototype.filter.call(lk, (e) => e.getAttribute("rel").iO(v));
      }
    }
      ...
      constructor() {
        ...
        let _allScripts = ($t) => $t.forEach((e) => _iScript(e)),
          ...
          _classAlways = ($t) =>
            ...
            Hints($.s.alwayshints).find(url),
          _iScript = ($S) => {
            url = $S.getAttribute(PK);

            if (PK == "href")
              return qs("head").appendChild($.parse(linki.replace("*", url)));
          ...
          _removeScript = () =>
            qa((PK == "href" ? linkr : scrr).replace("!", url)).forEach((e) =>
              e.parentNode.removeChild(e)
            );
      }
    }
      ...
      constructor() {
        ...
        this.a = function (o, p, t) {
          if (o === "=") {
            if (p)
              return (
                h === $.currentURL || h === l //check whether internally stored "href" ("h") variable is the same as the global currentURL
          ...
          if (o === "c")
            return can && can !== p && !p.iO("#") && !p.iO("?") ? can : p; //get internal "can" ("href" of canonical URL)
        };
        let _setE = (p, t) =>
          (h =
            typeof (e = p) !== "string"
          ...
          if (o === "a")
            divs.forEach((div) => {
              ...
              Array.prototype.filter
                .call(qa($.s.forms, div), function (e) {
                  //filter forms
    ...
    class Pronto {
      constructor() {
        ...
        let _init_p = () => {
          ...
          _noAjaxy = () => {
            const selectors =
              typeof $.s.selectorskip === "string" &&
              $.s.selectorskip.length > 0
                ? $.s.selectorskip.split(", ")
                : false;
          ...
          _prefetch = (t, e) => {
            ...
            if (!$.Rq("?", true)) return;
            var href = $.Rq("v", e, t);
            if ($.Rq("=", true) || !href || Hints($.s.prefetchoff).find(href))
              return;
            $.fn("+", href, () => false);
          },
          _stopBubbling = (e) => (
          ...
          _doRender = () => {
            $.trigger("load");
            if ($.s.bodyClasses) {
              var classes = $.fn("body").getAttribute("class");
              bdy.setAttribute("class", classes ? classes : "");
            }
            ...
            if ($.s.passCount)
              qs("#" + $.s.passCount).innerHTML = "Pass: " + $.pass;
            if ($.s.cb) $.s.cb();
